...
public class salaryRelatory extends JPanel {
    ...
    private CategoryDataset createGraphicalData() {
        ...
        try {
            ...
            selectionQuery += "FROM availableProfessionals";
            salaryRelatorySQLOutput = salaryRelatorySQLInstruction.executeQuery(selectionQuery);

            while (salaryRelatorySQLOutput.next()) {
                graphicalReceivedData.addValue(salaryRelatorySQLOutput.getInt("salaryIndex01"), "Until R$1.000,00", " < R$1.000,00");
                graphicalReceivedData.addValue(salaryRelatorySQLOutput.getInt("salaryIndex02"), "Between R$1.000,00 and R$2.000,00", "R$1.000,00 - R$2.000,00");
                graphicalReceivedData.addValue(salaryRelatorySQLOutput.getInt("salaryIndex03"), "Between R$2.000,00 and R$3.000,00", "R$2.000,00 - R$3.000,00");
                graphicalReceivedData.addValue(salaryRelatorySQLOutput.getInt("salaryIndex04"), "Between R$3.000,00 and R$4.000,00", "R$3.000,00 - R$4.000,00");
                graphicalReceivedData.addValue(salaryRelatorySQLOutput.getInt("salaryIndex05"), "Between R$4.000,00 and R$5.000,00", "R$4.000,00 - R$5.000,00");
            }
            return graphicalReceivedData;   
        } catch(SQLException salaryRelatorySQLException){
            JOptionPane.showMessageDialog(null, "Error D-01! " + salaryRelatorySQLException.getMessage());
